##########################
#filename:simulation_M**.py
#description:
#   a. this program implements M** (proposed model)
        ...
        for b in range(len(b_coef)):
            for a in range(len(a_coef)):
                for l in range(len(lr_rate)):
                    subject = subjects[s]
                    tkst_test = snscov_model(K, T, D, la_rate = lr_rate[l], amp=amp,tol=0.0,
...
np.save('../../results/proposed_20_replica_{}_distance_metric.npy'.format(args.waveform, distance_metric), distance_metric)
