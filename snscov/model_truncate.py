...
from snscov.evaluate import dual_orthogonal_procrustes, dual_permutation,large_scale_permutation, ortogonal_procrustes


def project_to_kernel(x, w,v, gamma, amp):
   
    #ub = np.ones(k_matrix.shape[1]) * amp
    #w,v = np.linalg.eigh(k_matrix)
    u = np.einsum('ij,j->i', v.T, x)
    #ub = np.einsum('ij,j->i', v.T, ub)
    k_matrix = v @ np.diag(w) @ v.T
...
class snscov_model:
    def __init__(self, K, T, D, w= None,v=None, max_iter = 1000, ini_method = 'spectral',amp=3.0,
    ...
    def update_alphas(self, X, dictionary, alphas):
        ...
        if self.a_method == 'temporal_kernel':
            ...
            for col in range(new_alphas.shape[1]):
                new_alphas[:,col]=project_to_kernel(new_alphas[:,col], self.w, self.v, self.smooth, self.amp)
    ...
    def update_dictionary(self, X, dictionary, alphas):
        ...
        q,r = np.linalg.qr(new_dictionary.T)
        new_dictionary = q[:,0:self.K].T
    ...
    def fit(self, X, tol=0., true_a=None, true_d=None, evaluate=False):
        ...
        if evaluate == True:
            self.dual_ortho.append(ortogonal_procrustes(true_a.T, true_d, alphas, dictionary))
        ...
        for ii in range(self.max_iter):
            ...
            if evaluate == True:
                ...
                if ii>0:
                    if ((self.error_[ii+1] - self.error_[ii])>self.tol): 
                        break
                    else:
                        self.best_alphas = alphas
                        self.best_dictionary = dictionary
