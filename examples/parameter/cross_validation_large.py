...
#tuning parameter
gamma = np.array([0.1]) #[100]
length_scale = np.array([5,10,50,100])
sparse_level = np.array([10,25,30,60,80])
            ...
            for ls in range(len(length_scale)):
                for kk in range(len(K_set)):
                    ...
                    for k in range(k_fold):
                        ...
                        tkst_test = snscov_model(K_set[kk], T, D, la_rate = lr_rate[lr], amp=amp,tol=0.1,
...
np.savez('../../parameters/{}_sparse_ms.npz'.format(args.filename),oos_mse=oos_mse_m, metric=eva_metric, aic_v=aic_v_n,bic_v=bic_v_n)